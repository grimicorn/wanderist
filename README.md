...
- `POST /api/connections/instagram/import` — pulls recent geotagged media, stores images in Netlify Blobs, and creates journal entries with linked places. Refreshes the stored token first when it is near expiry so imports self-heal.
